...
function documentLoader() {
    ...
    Promise.all([
      fetch('/folioDATA/'+folio_xml).then(response => response.text()),
      fetch("/text.xsl").then(response => response.text())
    ])
...
// function to transform the metadate encoded in teiHeader with the xsl stylesheet "Frankenstein_meta.xsl", this will apply the templates and output the text in the html <div id="stats">
  function statsLoader(){
    ...
    Promise.all([
      ...
      fetch('/folioDATA/'+folio_xml).then(response => response.text()),
      fetch("/meta.xsl").then(response => response.text())
    ])
    .then(function ([xmlString, xslString]) {
      var parser = new DOMParser();
      var xml_doc = parser.parseFromString(xmlString, "text/xml");
  ...
  var PercyArray = Array.from(visible_percy);
    ...
    //write an forEach() method that shows all the text written and modified by both hand (in black?). The forEach() method of Array instances executes a provided function once for each array element.
     MaryArray.forEach((MaryElement) => {MaryElement.style.color = 'black';});
     PercyArray.forEach((PercyElement) => {PercyElement.style.color = 'black';});
     ...
     MaryArray.forEach((MaryElement) => {MaryElement.style.color = 'lightgrey';});
...
// write another function that will toggle the display of the deletions by clicking on a button
  ...
  del_Array.forEach((DelElement) => {DelElement.style.fontWeight = 'lightgrey';});
  del_Array.forEach((DelElement) => {DelElement.style.color = 'crimson';});
  }
  else {
...
// EXTRA: write a function that will display the text as a reading text by clicking on a button or another dropdown list, meaning that all the deletions are removed and that the additions are shown inline (not in superscript)
  function ReadingText() {
    var del_vision = document.getElementsByTagName('del');
    var del_Array = Array.from(del_vision);
    del_Array.forEach((DelElement) => {DelElement.style.display = 'none';});
    
    var add_vision = document.getElementsByTagName('sup');
    var add_Array = Array.from(add_vision);
    add_Array.forEach((AddElement) => {
      var parent = AddElement.parentNode;
      while (AddElement.firstChild) {
        parent.insertBefore(AddElement.firstChild, AddElement);
      }
      parent.removeChild(AddElement);
    });
  }
